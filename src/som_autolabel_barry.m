function [sTo] = som_autolabel_barry(sTo, sFrom, mode, inds)
...
label_names = unique(sFrom.labels);

for i = 1:size(label_names)
    
    % Find the labels in the SOM that match the labeled sample.
    count = 1;
    clear label_indices;
    clear label_freqs;
    clear label_freq_string;
    for j = 1:size(sTo.labels(:))
       if ~isempty(sTo.labels{j}) & strcmp(label_names{i}, '') == 0  & regexp(sTo.labels{j}, strcat(label_names{i},'(\d*)')) ~= 0
           label_freq_string = regexp(sTo.labels{j}, '(\d*)', 'match');
           label_freqs(count) = str2num(label_freq_string{1});
           
           label_indices(count) = j;
           count = count + 1;
       end
    end
    
    % Delete least frequent labels from the SOM
    if exist('label_indices') & exist('label_freqs') & size(label_freqs,2) > 2
        [label_freqs_sorted sort_order] = sort(label_freqs, 'descend');
        label_indices_to_delete = label_indices(sort_order(2:end));
        for k = 1:size(label_indices_to_delete,2)
            sTo.labels{label_indices_to_delete(k)} = '';
        end
    end
end

% % Fix the label cell array...
% for i = 1:size(sTo.labels(:))
%     
% end
