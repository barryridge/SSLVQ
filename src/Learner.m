        ...
        function [TrainingData TestData] = setupdatastructs(Data, varargin)
            ...
            if epochs > 1
                ...
                for i = 2:epochs
                    ...
                    if isfield(TrainingData, 'NormedFeatureVectors')
                        TrainingData.NormedFeatureVectors =...
                            [TrainingData.NormedFeatureVectors TempTrainingData.NormedFeatureVectors];
                    end
                    ...
                    if isfield(TrainingData, 'FileNames')
                        TrainingData.FileNames =...
                            {TrainingData.FileNames{:} TempTrainingData.FileNames{:}};
                    end
                    ...
                    if isfield(Data, 'Modalities')
                        for iMod = 1:length(Data.Modalities)
                            ...
                            if isfield(TrainingData.Modalities{iMod}, 'FeatureVectors')
                            ...
                            if isfield(TrainingData.Modalities{iMod}, 'ClassLabels')
                            ...
                            if isfield(TrainingData.Modalities{iMod}, 'FileNames')
                                TrainingData.Modalities{iMod}.FileNames =...
                                    {TrainingData.Modalities{iMod}.FileNames{:} TempTrainingData.Modalities{iMod}.FileNames{:}};
                            end
            ...
            switch Method
                ...
                case 'samplewise',                    
                    ...
                    for i = 1:size(RandIndices,2)                        
                        ...
                        if isfield(RandomizedData, 'FeatureVectors')
                            RandomizedData.FeatureVectors(:,i) = Data.FeatureVectors(:,RandIndices(i));
                        end
                        
                        if isfield(RandomizedData, 'NormedFeatureVectors')
                            RandomizedData.NormedFeatureVectors(:,i) =...
                                Data.NormedFeatureVectors(:,RandIndices(i));
                        end
                        
                        if isfield(RandomizedData, 'ClassLabels')
                            RandomizedData.ClassLabels(:,i) = Data.ClassLabels(:,RandIndices(i));
                        end
                        
                        if isfield(RandomizedData, 'FileNames')
                            RandomizedData.FileNames{i} = Data.FileNames{RandIndices(i)};
                        end
                        ...
                        if isfield(RandomizedData, 'Modalities')
                            for j = 1:length(RandomizedData.Modalities)
                                ...
                                if isfield(RandomizedData.Modalities{j}, 'FileNames')
                                    RandomizedData.Modalities{j}.FileNames{i} = Data.Modalities{j}.FileNames{RandIndices(i)};
                                end
                            end
                        end
                ...
                case 'classwise',
                    ...
                    for iClass = 1:size(RandClasses,2)
                        ...
                        if ~isempty(ClassIndices)
                            ...
                            if isfield(RandomizedData, 'NormedFeatureVectors')
                                RandomizedData.NormedFeatureVectors(:,iIn:iIn+(inc-1)) = Data.NormedFeatureVectors(:,ClassIndices);
                            end
                            ...
                            if isfield(RandomizedData.Modalities{j}, 'FileNames')
                                iRand = 1;
                                for k = iIn:iIn+(inc-1)
                                    iClassIndices = iClassIndices + 1;
                                    RandomizedData.FileNames{k} =...
                                        Data.FileNames{ClassIndices(iClassIndices)};
                                end
                            end
                            ...
                            if isfield(RandomizedData, 'Modalities')
                                for j = 1:length(RandomizedData.Modalities)
                                    ...
                                    if isfield(RandomizedData.Modalities{j}, 'FileNames')
                                        ...
                                        for k = iIn:iIn+(inc-1)
                                            iClassIndices = iClassIndices + 1;
                                            RandomizedData.Modalities{j}.FileNames{k} =...
                                                Data.Modalities{j}.FileNames{ClassIndices(iClassIndices)};
                                        end
                                    end
                                end
                            end
