classdef SVMLearner < Learner
    % SVMLEARNER Summary of this class goes here
    ...
    methods
        ...
        %******************************************************************
        %******************************************************************
        function obj = SVMLearner(varargin)
...
%             end
        ...
        function obj = train(obj, varargin)
            ...
            while i <= length(varargin), 
                ...
                if ~argok, 
                    disp(['SVMLearner.train(): Ignoring invalid argument #' num2str(i)]);
                    fprintf(obj.UsageMessage);
                end
            ...
            training_label_vector = ones(size(obj.TrainingData1Epoch.Modalities{1}.ClassLabels,2),1);            
            for iClass = 2:obj.TrainingData1Epoch.Modalities{1}.nGroundTruths
                training_label_vector(logical(obj.TrainingData1Epoch.Modalities{1}.ClassLabels(...
                                        obj.TrainingData1Epoch.Modalities{1}.GroundTruthLabelIndices(iClass), :)), :) = iClass;
            end
                  
            training_instance_matrix = obj.TrainingData1Epoch.Modalities{1}.NormedFeatureVectors';
            ...
            % Training the model using the optimized parameters...
            cmd = ['-q -c ', num2str(bestc), ' -g ', num2str(bestg)];
            obj.Model = svmtrain(training_label_vector, training_instance_matrix, cmd);
        ...
        function [TestData Mask] = classify(obj, varargin)        
            ...
            testing_instance_matrix = TestData.Modalities{1}.NormedFeatureVectors';
            
            [predicted_label, accuracy, decision_values] = svmpredict(testing_label_vector, testing_instance_matrix, obj.Model, [, 'libsvm_options']);

            TestData.Results.InToOutClassification = predicted_label';
