function h=CrossMod_som_cplane(varargin) 
...
if length(pos) == 3
    z=repmat(ones(size(unit_coords(:,1)')) * pos(3), l, 1);
else
    z=repmat(zeros(size(unit_coords(:,1)')), l, 1);
end
...
if ~isnan(color)
   h_=patch(x,y,z,color);         
else
   h_=patch(x,y,z,'k');                     % empty grid 
