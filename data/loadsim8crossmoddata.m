%
% This is based on the dataset described in
...
% but we expand the original single-dimensional dataset into the
% cross-modal domain by creating a second modality with three easily
% seperable Gaussian clusters, one for each class.
%
%
function Data = loadsim8crossmoddata(varargin)
    ...
    Foo = gencluster([0.28 0.15], [0.2, 0.2], [1, 1], [0, 0], (sample_size/3)/2, 1);
    Foo = [Foo' gencluster([0.65 0.075], [0.1, 0.1], [1, 1], [0, 0], (sample_size/3)/2, 2)']';
    Foo = [Foo' gencluster([0.1 0.65], [0.2, 0.15], [1, 1], [0, 0], (sample_size/3)/2, 2)']';
    Foo = [Foo' gencluster([0.55 0.95], [0.2, 0.15], [1, 1], [0, 0], (sample_size/3)/2, 3)']';
    Foo = [Foo' gencluster([0.75 0.6], [0.15, 0.075], [1, 1], [0, 0], (sample_size/3)/2, 1)']';
    Foo = [Foo' gencluster([0.85 0.65], [0.15, 0.06], [1, 1], [0, 0], (sample_size/3)/2, 3)']';
    ...
    Mod1XClass1 = Foo(Foo(:,end)==1,1:end-1);
    Mod1XClass2 = Foo(Foo(:,end)==2,1:end-1);
    Mod1XClass3 = Foo(Foo(:,end)==3,1:end-1);    
    ...
    % Modality 1:
    Data.Modalities{1}.FeatureIndices = 1:size(Mod1XClass1,2);
    Data.Modalities{1}.FeatureMask = zeros(1,size(Data.FeatureVectors,1));
    Data.Modalities{1}.FeatureMask(1:size(Mod1XClass1,2)) = 1;
    % Modality 2:
    Data.Modalities{2}.FeatureIndices = size(Mod1XClass1,2)+1:size(Data.FeatureVectors,1);
    Data.Modalities{2}.FeatureMask = zeros(1,size(Data.FeatureVectors,1));
    Data.Modalities{2}.FeatureMask(size(Mod1XClass1,2)+1:size(Data.FeatureVectors,1)) = 1;
