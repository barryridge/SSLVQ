...
% 
%
function Data = loadsim6crossmoddata(varargin)
    ...
    Mod1XClass1 = rand2DGaussian([0.5 0.5],[0.01 0;0 0.01],sample_size/2)';    
    Mod1XClass2 = rand2DGaussian([0.5 0.5],[0.01 0;0 0.01],sample_size/2)';
    Mod2XClass1 = rand2DGaussian([0.25 0.5],[0.005 0;0 0.01],sample_size/2)';
    Mod2XClass2 = rand2DGaussian([0.75 0.5],[0.005 0;0 0.01],sample_size/2)';
    YClass1 = ones(sample_size/2,1);
    YClass2 = 2 * ones(sample_size/2,1);
    
    % Cut the Mod 1 class Gaussians in half...
    Mod1XClass1Split = Mod1XClass1(Mod1XClass1(:,1) < 0.5, :);    
    Mod1XClass2Split = Mod1XClass2(Mod1XClass2(:,1) > 0.5, :);
    
    % Delete the corresponding points in the second modality...
    Mod2XClass1Split = Mod2XClass1(Mod1XClass1(:,1) < 0.5, :);
    Mod2XClass2Split = Mod2XClass2(Mod1XClass2(:,1) > 0.5, :);        
    
    % And the corresponding class labels...
    YClass1Split = YClass1(Mod1XClass1(:,1) < 0.5);
    YClass2Split = YClass2(Mod1XClass2(:,1) > 0.5);
            
    %% Build the Data struct...
    X = [Mod1XClass1Split' Mod1XClass2Split'; Mod2XClass1Split' Mod2XClass2Split']';
    Y = [YClass1Split' YClass2Split']';        
